...
namespace ProductSite.Web.Services {
  public class ProductService : ServiceBase {
    ...
    public List<Product> GetProductByType(string type) {
      switch (type) {
        case "HotDeals":
          return base.db.Products.Where(p => p.HotDeal == true).ToList();
        case "NewArrivals":
          return base.db.Products.Where(p => p.NewArrival == true).ToList();
        case "Sold":
          return base.db.Products.Where(p => p.Sold == true).ToList();
        default:
          return null;
      }
    }
    ...
    public int ToggleProductSold(int productId, bool sold) {
      SubSonicRepository<Product> repository = base.GetRepository<Product>();
